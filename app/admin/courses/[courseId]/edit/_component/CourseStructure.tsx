...
export default function CourseBuilder() {
    ...
    setSections((prev) =>
      prev.map((s) =>
        s.id === sectionId
          ? {
              ...s,
              lessons: [...s.lessons, { id: nextId("les"), title: "New lesson" }],
            }
          : s,
      ),
    );
  ...
  const onSectionDragStart = (e: DragEvent<HTMLDivElement>, sectionId: string) => {
    e.dataTransfer.effectAllowed = "move";
    setDragBoth({ kind: "section", sectionId });
  };

  const onSectionDragOver = (e: DragEvent<HTMLDivElement>, sectionId: string) => {
  ...
  const onSectionDrop = (e: DragEvent<HTMLDivElement>, targetSectionId: string) => {
    if (dragRef.current?.kind !== "section") return;
    e.preventDefault();
    e.stopPropagation();
  ...
  const onLessonDragStart = (e: DragEvent<HTMLDivElement>, sectionId: string, lessonId: string) => {
    e.stopPropagation();
    e.dataTransfer.effectAllowed = "move";
    setDragBoth({ kind: "lesson", sectionId, lessonId });
  };

  const onLessonDragOver = (e: DragEvent<HTMLDivElement>, sectionId: string, lessonId: string) => {
  ...
  const onLessonListDragOver = (e: DragEvent<HTMLDivElement>, sectionId: string) => {
    if (dragRef.current?.kind !== "lesson") return;
    e.preventDefault();
    e.stopPropagation();
  ...
  const commitLessonDrop = (
    ...
    e.preventDefault();
    e.stopPropagation();
    const { sectionId: sourceSectionId, lessonId: sourceLessonId } = dragRef.current;
    ...
    setSections((prev) => {
      ...
      const srcLessonIdx = next[srcSectionIdx].lessons.findIndex((l) => l.id === sourceLessonId);
  ...
  const onLessonDrop = (e: DragEvent<HTMLDivElement>, sectionId: string, lessonId: string) =>
      ...
      <Card className="py-0">
        <CardHeader className="flex items-center gap-2 py-6! mb-4 border-b">
          ...
          <span className="text-xs ml-auto">
            Drag <GripVertical className="inline w-3 h-3 -mt-0.5" /> handles to reorder
          </span>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col gap-3">
            {sections.map((section) => {
              ...
              const showBeforeLine =
                over?.kind === "section" && over.id === section.id && over.position === "before";
              const showAfterLine =
                over?.kind === "section" && over.id === section.id && over.position === "after";

              return (
                <div key={section.id}>
                  {showBeforeLine && <div className="h-0.5 rounded-full mb-1" />}
                      ...
                      >
                        {section.lessons.map((lesson) => {
                          const isDraggingThisLesson =
                            drag?.kind === "lesson" && drag.lessonId === lesson.id;
                          ...
                          return (
                            <div key={lesson.id}>
                              {lessonShowBefore && <div className="h-0.5  rounded-full mb-1" />}
                              <div
                                className={`flex items-center gap-2 px-2 py-1.5 rounded-lg border cursor-grab active:cursor-grabbing transition-opacity ${
                                  isDraggingThisLesson ? "opacity-40" : "opacity-100  "
                                }`}
                                draggable
                                onDragStart={(e) => onLessonDragStart(e, section.id, lesson.id)}
                                onDragOver={(e) => onLessonDragOver(e, section.id, lesson.id)}
                                onDrop={(e) => onLessonDrop(e, section.id, lesson.id)}
                                onDragEnd={clearDrag}
                              >
                                ...
                                <Input
                                  value={lesson.title}
                                  onChange={(e) =>
                                    renameLesson(section.id, lesson.id, e.target.value)
                                  }
                                  className="flex-1 text-sm bg-transparent! outline-none! rounded px-1 border-none! focus-visible:border-none! focus-visible:ring-0! -mx-1"
                                />
                                <Button
                        ...
                        {over?.kind === "lesson" &&
                          over.sectionId === section.id &&
                          over.id === "__end__" && <div className="h-0.5 rounded-full" />}
                  ...
                  {showAfterLine && <div className="h-0.5 bg-indigo-500 rounded-full mt-1" />}
                </div>
              );
            })}
          </div>
