...
function RenderUploadedContent({
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      {type === "image" ? (
        <>
          <Image src={previewUrl} alt="Preview" fill className="object-contain p-2 relative" />
...
function RenderUploadingState({ progress, file }: { progress: number; file: File }) {
  return (
    <div className="text-center flex justify-center items-center flex-col">
      ...
      <p className="text-xs text-muted-foreground truncate mt-1 max-w-xs">{file.name}</p>
    </div>
  );
}
